    ...
    public class CD_Cliente
    {
        public int Registrar(Cliente obj, out string Mensaje)
        {
            ...
            try
            {
                ...
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    ...
                    cmd.Parameters.AddWithValue("CLI_NOMBRES", obj.CLI_NOMBRES);
        ...
        public List<Cliente> Listar()
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    ...
                    string query = "select CLI_ID,CLI_NOMBRES,CLI_APELLIDOS,CLI_CORREO,CLI_CLAVE,CLI_REESTABLECER from CLIENTE";
        ...
        public bool CambiarClave(int idcliente, string nuevaclave, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("update CLIENTE set CLI_CLAVE = @nuevaclave , CLI_REESTABLECER = 0 where CLI_ID = @id", oconexion);
        ...
        public bool ReestablecerClave(int idcliente, string clave, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("update CLIENTE set CLI_CLAVE= @clave , CLI_REESTABLECER = 1 where CLI_ID = @id", oconexion);
                    cmd.Parameters.AddWithValue("@id", idcliente);
